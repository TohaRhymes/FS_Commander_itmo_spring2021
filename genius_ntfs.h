...
struct ntfs_bpb {
    ...
    uint8_t sec_per_clust; // The number of sectors in a cluster. Clusters are used for less-granular access to the partition. They're usually 4KB.
    ...
    uint64_t mft_lclust; // The start of the MFT, given as a cluster index.
...
struct ntfs_mft_record {
    ...
    uint16_t seq_no;
    uint16_t link_count;
    uint16_t attrs_offset; // Number of bytes between the start of the header and the first attribute header.
    ...
    uint16_t reserved;
    uint32_t mft_record_no; // The record number.
} __attribute__((__packed__));   /* 48 bytes */

struct ntfs_attr_record {
    uint32_t type;      // The type of attribute. We'll be interested in $DATA (0x80), and $FILE_NAME (0x30). /* Attr. type code */
    uint32_t len;   // The length of the attribute in the file record.
    uint8_t non_resident;    // false = attribute's contents is stored within the file record in the MFT; true = it's stored elsewhere
    uint8_t name_len;
    ...
    union {
        ...
        struct {    /* Non-resident attributes */
            uint64_t lowest_vcn; //firstCluster;
            uint64_t highest_vcn; //lastCluster
            uint16_t mapping_pairs_offset; // The offset in bytes from the start of attribute header to the description of where the attribute's contents can be found.
...
int read_clusters2buf(uint8_t **buf, uint64_t *buf_current_size, uint64_t *buf_size, int64_t lcn, uint64_t length,
